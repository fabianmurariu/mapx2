...
/// ```
pub struct ByteStore<T: AsMut<[u8]>> {
    buffer: T,
...
impl<T: AsMut<[u8]> + AsRef<[u8]>> ByteStore<T> {
...
mod tests {
    ...
    fn test_capacity_limit() {
        let mut store = ByteStore::new(vec![0u8; 32]);
    ...
    proptest! {
        ...
        fn prop_test_store_retrieval(
            ...
            let mut store = ByteStore::new(vec![0u8; 8192]);
            let mut indices = Vec::new();
        ...
        fn prop_test_no_out_of_bounds_access(
            ...
            let mut store = ByteStore::new(vec![0u8; 4096]);
        ...
        fn prop_test_free_space_calculation(
            ...
            let mut store = ByteStore::new(vec![0u8; buffer_size]);
