                    ...
                    if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                        if let Some(slab_name) = file_name.strip_suffix(".bin") {
                            if let Some(slab_id_str) = slab_name.strip_prefix("slab") {
                                if let Ok(slab_id) = slab_id_str.parse::<usize>() {
                                    if slab_id < SLAB_SIZES.len() {
...
impl HeapOps<VecStore> for Heap<VecStore> {
    fn get_or_create_slab(&mut self, category: usize) -> &mut Slab<VecStore> {
        if self.slabs[category].is_none() {
            let element_size = SLAB_SIZES[category];
            let store = VecStore::new();
            let slab = Slab::new(store, element_size);
            self.slabs[category] = Some(slab);
        }

        self.slabs[category].as_mut().unwrap()
    }
}

impl HeapOps<MMapFile> for Heap<MMapFile> {
    fn get_or_create_slab(&mut self, category: usize) -> &mut Slab<MMapFile> {
        if self.slabs[category].is_none() {
            let element_size = SLAB_SIZES[category];
