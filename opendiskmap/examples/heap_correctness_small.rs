...
fn generate_random_data_chunk(count: usize, min_size: usize, max_size: usize, rng: &mut StdRng) -> Vec<Vec<u8>> {
...
fn main() {
    ...
    println!("Average insertion rate: {:.0} items/sec", final_count as f64 / insertion_time.as_secs_f64());
    ...
    for (index, (original_size, expected_hash)) in &verification_map {
        if let Some(retrieved_data) = heap.get(*index) {
            ...
            if trimmed_data.len() != *original_size {
                eprintln!("Size mismatch for index {:?}: expected {}, got {}", 
                         index, original_size, trimmed_data.len());
                errors += 1;
                continue;
            }
    ...
    println!("Average verification rate: {:.0} items/sec", verified_count as f64 / verification_time.as_secs_f64());
    
    println!("\n=== Overall Results ===");
    println!("Total time: {:.2}s", total_time.as_secs_f64());
    println!("Success rate: {:.2}%", (verified_count as f64 / final_count as f64) * 100.0);
    
    if errors == 0 {
        println!("✅ ALL TESTS PASSED! Heap correctness verified for {} items.", verified_count);
    } else {
